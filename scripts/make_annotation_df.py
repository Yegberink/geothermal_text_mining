...
def main() -> None:
    ...
    required_cols = {"sentence_uid", "sentence_text", "aspect"}
    ...
    dedupe_subset = [c for c in ["sentence_text", "aspect", "sentiment"] if c in out.columns]
    if dedupe_subset:
        out = out.drop_duplicates(subset=dedupe_subset).copy()
    else:
        out = out.drop_duplicates(subset=["sentence_uid"]).copy()
